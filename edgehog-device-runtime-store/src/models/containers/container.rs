...
pub struct Container<'a> {
    ...
    pub cpuset_cpus: Option<Cow<'a, str>>,
...
/// CGroup name-space mode
#[repr(u8)]
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, FromSqlRow, AsExpression)]
#[diesel(sql_type = SmallInt)]
pub enum CgroupnsMode {
    /// Unset cgroup namespace mode
    Empty = 0,
    /// Run the container in its own private cgroup namespace
    Private = 1,
    /// Run the container in the host cgroup namespace
    Host = 2,
}
...
pub struct ContainerGroupAdd<'a> {
    /// [`Container`] id
    pub container_id: SqlUuid,
    /// Additional group to add to the container process.
...
mod tests {
    ...
    #[case("always", ContainerRestartPolicy::Always)]
