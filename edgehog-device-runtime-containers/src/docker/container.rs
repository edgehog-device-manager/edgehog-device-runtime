...
impl Container {
    ...
    /// Normalizes the container for what the container runtime expects.
    pub(crate) fn normalize(&mut self) {
        // We set the health-check struct when converting from the stored container, but it requires
        // the command to also be set.
        self.req
            .healthcheck
            .take_if(|h| h.test.as_ref().is_none_or(|t| t.is_empty()));
    }
