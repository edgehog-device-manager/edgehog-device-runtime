...
pub(crate) mod tests {
    ...
    pub fn create_network_request_event(network: &CreateNetwork) -> DeviceEvent {
        let fields = [
            ...
            (
                "internal",
                AstarteData::Boolean(network.internal.unwrap_or_default()),
            ),
            (
                "enableIpv6",
                AstarteData::Boolean(network.enable_ipv6.unwrap_or_default()),
            ),
            (
                "options",
                AstarteData::StringArray(network.options.clone().unwrap_or_default()),
            ),
        ]
        .into_iter()
