...
pub fn notify(event: impl IntoSecurityEvent) {
...
/// A trait for types that can be converted or mapped into a [`SecurityEvent`].
pub trait IntoSecurityEvent {
    /// Attempts to convert the implementing type into a [`SecurityEvent`].
    fn into_event(self) -> Option<SecurityEvent>;
}

impl IntoSecurityEvent for SecurityEvent {
    fn into_event(self) -> Option<SecurityEvent> {
        Some(self)
    }
}
