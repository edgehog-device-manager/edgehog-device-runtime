...
const INTERFACE: &str = "io.edgehog.devicemanager.apps.AvailableFileBinds";

/// Available file bind property.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct AvailableFileBind<'a> {
    id: &'a Uuid,
}

impl<'a> AvailableFileBind<'a> {
    pub(crate) fn new(id: &'a Uuid) -> Self {
        Self { id }
    }
}

impl AvailableProp for AvailableFileBind<'_> {
    type Data = bool;
...
mod tests {
    ...
    #[tokio::test]
    async fn should_store_file_bind() {
        let id = Uuid::new_v4();

        let file_bind = AvailableFileBind::new(&id);
        ...
        file_bind.send(&mut client, true).await.unwrap();
    }

    #[tokio::test]
    async fn should_unset_file_bind() {
        let id = Uuid::new_v4();
        ...
        file_bind.unset(&mut client).await.unwrap();
    }
}
