...
impl<'a> TryFrom<&'a CreateContainer> for container::Container<'a> {
    ...
    fn try_from(value: &'a CreateContainer) -> Result<Self, Self::Error> {
        ...
        let cpu_period = cpu_period.filter(|v| *v >= 0);
        let cpu_quota = cpu_quota.filter(|v| *v >= 0);
        ...
        // Set default to 0 to fix a bug in podman 5.8.7, which set's it to a value bigger than the
        // allowed i64
        let cpu_period = cpu_period.unwrap_or(0);

        let cpu_realtime_period = cpu_realtime_period.filter(|v| *v >= 0);
        let cpu_realtime_runtime = cpu_realtime_runtime.filter(|v| *v >= 0);
        let memory = memory.filter(|v| *v >= 0);
        let memory_reservation = memory_reservation.filter(|v| *v >= 0);
        // -1 is a value
        let memory_swap = memory_swap.filter(|v| *v >= -1);
