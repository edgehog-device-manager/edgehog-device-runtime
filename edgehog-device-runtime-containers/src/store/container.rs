...
impl StateStore {
    ...
    pub(crate) async fn create_container(&self, container: CreateContainer) -> Result<()> {
...
fn create_container(
    ...
    let memory_reservetion = QuotaValue::<-1>::new(memory_reservetion);
    ...
    match (cpu_period, cpu_quota) {
        ...
        (None, Some(_)) => {
            return Err(StoreError::Conversion {
                ctx: format!("cpu period missing, while cpu quota is set"),
            })
        }
        (Some(_), None) => {
            return Err(StoreError::Conversion {
                ctx: format!("cpu period was set, while cpu quota is missing"),
            })
        }
    }
...
mod tests {
    ...
    async fn should_create_full() {
        ...
        let exp = Container {
            ...
            memory_reservetion: QuotaValue::new(1024),
