...
impl AstarteMessageHubNode {
    pub async fn new(options: AstarteMessageHubNodeOptions) -> Result<Self, DeviceManagerError> {
        ...
        let interface_json = read_interfaces_from_directory(&options.interfaces_directory)?;
...
impl TryFrom<&DeviceManagerOptions> for AstarteMessageHubNodeOptions {
    ...
    fn try_from(opts: &DeviceManagerOptions) -> Result<Self, Self::Error> {
        let endpoint = opts
            ...
            .as_ref()
            .ok_or_else(|| {
                DeviceManagerError::AstarteMessageHubOptions(
                    "Unable to find MessageHub options".to_string(),
                )
            })?
            .endpoint
            .clone();
...
mod tests {
    ...
    #[test]
    fn try_astarte_message_options_from_empty_options() {
    ...
    #[test]
    fn try_astarte_message_options_from_valid_options() {
        let device_options = DeviceManagerOptions {
            astarte_library: AstarteLibrary::AstarteDeviceSDK,
            astarte_device_sdk: None,
        ...
        let astarte_options = result_try.unwrap();
        assert_eq!(astarte_options.endpoint, "1".to_string());
        assert_eq!(astarte_options.interfaces_directory, "/tmp".to_string())
    }
