...
impl<F> FileTransfer<F> {
    async fn write_chunks(
        ...
        let written = file
            ...
            .await?;
        let mut written = usize::try_from(written).unwrap_or(usize::MAX);

        let complete_len = usize::try_from(file.range().complete_len()).unwrap_or(usize::MAX);
        ...
        {
            let max_write = cmp::min(bytes.len(), complete_len.saturating_sub(written));
