...
impl<F> FileStorage<F> {
    ...
    #[instrument(skip_all)]
    async fn cleanup(&self, queue: &Queue) -> io::Result<()> {
            ...
            .inspect_err(|error| error!(%error,"couldn't get next dir entry"))
        {
            ...
            let cleanup = Self::should_cleanup_symlinks(&entry, &path).await
                || Self::should_cleanup_partial(&path, queue).await;
            ...
            if let Err(error) = tokio::fs::remove_file(&path).await {
                error!(%error, path = %path.display(), "couldn't cleanup entry");
            }
        }
    ...
    #[instrument(skip_all)]
    async fn should_cleanup_symlinks(entry: &DirEntry, path: &Path) -> bool {
    ...
    #[instrument(skip_all)]
    async fn should_cleanup_partial(path: &Path, queue: &Queue) -> bool {
    ...
    pub(crate) async fn open_read(
        ...
        let dir_path = self.dir_path(id);

        if !file_path.starts_with(&dir_path) {
            error!(file_path = %file_path.display(), dir_path = %dir_path.display(), "path received outside target uuid path");
            return Err(io::Error::new(
                io::ErrorKind::InvalidFilename,
                "path received outside target uuid path",
            ));
        }
