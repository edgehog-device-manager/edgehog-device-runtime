        ...
        C: Client + Send + Sync + 'static,
    {
        ...
        match interface {
            ...
            TelemetryInterface::Geolocation => {
                cfg_if::cfg_if! {
                    if #[cfg(all(feature = "zbus", target_os = "linux"))] {
                        let telemetry = super::stats::geolocation::GeolocationTelemetry::default();

                        task.run(telemetry).await;
                    } else {
                        tracing::warn!("the geolocation telemetry interface is not supported because the zbus feature is missing")
                    }
                }
            }
        }
